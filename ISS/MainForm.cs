    ...
    public partial class MainForm : Form
    {
        ...
        private void pictureSearch_Click(object sender, EventArgs e)
        {
            ...
            if(Transfer.Welcome == "admin")
            {
                ...
                {
                    MySqlCommand command = new MySqlCommand("SELECT * FROM `ships` WHERE `id`= @search OR `status`= @search OR `board number`= @search OR `owner`= @search OR `brand`= @search", DB.getConnection());
            ...
            else
            {
                ...
                MySqlCommand command = new MySqlCommand("SELECT `board number`,`owner`,`brand`,`start date`,`end date` FROM `ships` WHERE `board number`= @search", DB.getConnection());
                command.Parameters.Add("@search", MySqlDbType.VarChar).Value = request;
                adapter.SelectCommand = command;
            }
