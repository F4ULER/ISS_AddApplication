...
namespace ISS
{
    class DataBase
    {
        ...
        public bool AddNote(string status, string num, string owner, string brand, string startDate, string endDate, string table)
        {
            ...
            MySqlCommand command = new MySqlCommand("INSERT INTO `" + table + "`(`status`, `board number`, `owner`, `brand`, `start date`, `end date`) VALUES (@status, @num , @owner, @brand, @startDate , @endDate)", getConnection());
        ...
        {
            MySqlDataAdapter adapter = new MySqlDataAdapter();
            MySqlCommand command = new MySqlCommand("INSERT INTO `additional_services` (`board number`, `service`) VALUES (@num, @service);", getConnection());
            command.Parameters.Add("@num", MySqlDbType.VarChar).Value = num;
